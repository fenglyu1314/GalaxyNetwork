...
//
// 至此网页版的算法已经和 GalaxyNetwork.ush 对齐：
//   · 整数 hash（pcg2d ↔ INT_HASH 位运算）
//   · 圆周运动 + 独立相位 / 速度
//   · 亮度剔除 + 边 hash + 亮度·距离权重的概率筛
//   · 时间脉冲闪烁
//   · 连线远端的距离过渡
//
// 唯一相对 .ush 的视觉增强：远端连线"线宽随距离收窄"。
// .ush 中 LINE_ABS 的 smoothstep(1.44, 0.16, _len2) 只衰减强度，
// 这里把 w2 也按同一个权重缩，让远端线"尖出去"再消失，过渡更自然。
//
// 默认参数取"最像星空"的预设：稀疏星 + 暗淡基底 + 强亮峰闪烁。
// 第 8 节会把整套对应到 .ush 的 Custom Node 实现，做最终对照。
